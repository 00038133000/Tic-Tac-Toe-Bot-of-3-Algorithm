...
pygame.display.set_caption("Alpha Beta Pruning AI")
screen.fill(WHITE)
...
IMAGE_FOLDER = "images/alphabeta_results"
...
def save_result(result):
    ...
    with open("alphabeta_results.txt", "a") as file:
...
def save_time_memory(time_taken, memory_taken):
    timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H-%M-%S")
    with open("alphabeta_time_memory.txt", "a") as file:
...
# alphabeta algorithm with alpha-beta pruning
    ...
    if is_maximizing:
        ...
        for row in range(BOARD_ROWS):
            for col in range(BOARD_COLS):
                if board[row][col] == '':
                    board[row][col] = 'O'
                    score = alphabeta(depth + 1, False, alpha, beta)
                    board[row][col] = ''
        ...
        for row in range(BOARD_ROWS):
            for col in range(BOARD_COLS):
                if board[row][col] == '':
                    board[row][col] = 'X'
                    score = alphabeta(depth + 1, True, alpha, beta)
                    board[row][col] = ''
...
while running:
    ...
    if turn == 'O' and current_time - player_move_time >= 500:
        ...
        # Computer's turn using alphabeta algorithm with alpha-beta pruning
        start_time = time.time()  
        
        # Measure memory before the alphabeta call
        ...
        for row in range(BOARD_ROWS):
            for col in range(BOARD_COLS):
                if board[row][col] == '':
                    board[row][col] = 'O'
                    score = alphabeta(0, False, alpha, beta)
                    board[row][col] = ''
                    if score > best_score:
                        best_score = score
                        best_move = (row, col)
        ...
        # Measure memory after the alphabeta call
        after_memory = psutil.Process().memory_info().rss
        memory_usage = after_memory - before_memory
        alphabeta_memory = memory_usage / 1024  # Convert to kilobytes
        
        end_time = time.time()  # End timing
        alphabeta_time = end_time - start_time  # Calculate the time taken
        
        # Save result to text file
        save_time_memory(alphabeta_time, alphabeta_memory)
